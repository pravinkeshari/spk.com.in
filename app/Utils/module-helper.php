    ...
    function getTaxModuleSystemTypesConfig($getTaxVatList = true, $tax_payer = 'vendor'): array
    {
        ...
        return Cache::remember($cacheKey, now()->addMinutes(30), function () use ($getTaxVatList, $tax_payer) {
            if (getCheckAddonPublishedStatus('TaxModule')) {
                $systemTaxVat = \Modules\TaxModule\app\Models\SystemTaxSetup::where('is_active', 1)
                ...
                if (!$systemTaxVat) {
                    $systemTaxVat = \Modules\TaxModule\app\Models\SystemTaxSetup::create([
                        'tax_type' => 'order_wise',
                        'country_code' => null,
                        'tax_payer' => 'vendor',
                        'is_default' => true,
                ...
                if ($getTaxVatList) {
                    $taxVats = \Modules\TaxModule\app\Models\Tax::where('is_active', 1)->where('is_default', 1)->get();
                }
