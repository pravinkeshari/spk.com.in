...
class PaymentConfigController extends Controller
{
    ...
    public function payment_config_get()
    {
        ...
        $data_values = $this->payment_setting->whereIn('settings_type', ['payment_config'])->get()
            ->map(function ($item) {
                if (is_string($item->live_values)) {
                    $item->live_values = json_decode($item->live_values, true) ?: [];
                }
                if (is_string($item->test_values)) {
                    $item->test_values = json_decode($item->test_values, true) ?: [];
                }
                return $item;
            });
