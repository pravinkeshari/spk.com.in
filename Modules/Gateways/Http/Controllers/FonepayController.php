...
class FonepayController extends Controller
{
    ...
    {
        $config = $this->payment_config('fonepay', 'payment_config');
    ...
    /**
     * Initiate Fonepay payment (redirect form).
     */
    public function payment(Request $req): View|Application|Factory|JsonResponse|\Illuminate\Contracts\Foundation\Application
    {
        ...
        $returnUrl = !empty($config_val->return_url) ? $config_val->return_url : route('fonepay.callback');
        ...
        return view('Gateways::payment.fonepay', compact(
            'paymentUrl',
            'merchantCode',
            'md',
            'prn',
    ...
    /**
     * Fonepay callback (verify DV and mark payment).
     */
    public function callback(Request $request): Application|JsonResponse|Redirector|\Illuminate\Contracts\Foundation\Application|RedirectResponse
    {
        ...
        $success = filter_var($ps, FILTER_VALIDATE_BOOLEAN) && $dvMatch;
